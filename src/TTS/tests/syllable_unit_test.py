...
def run_test_cases(__scu, _allophones: [[Allophone]], _results: []):
    n = len(_allophones)
    for i in range(n):
        allophones = _allophones[i]
        results = _results[i]
        res = __scu.process_by_word(allophones)
...
class SyllableComplex3UnitCase(unittest.TestCase):
    ...
    def test_sonant_vowel_by_word(self):
        ...
        try:
            run_test_cases(self.__sc3u, allophones, results)
...
class SyllableComplex2UnitCase(unittest.TestCase):
    ...
    def _test_two_sonants_by_word(self):
        ...
        try:
            run_test_cases(self.__sc2u, allophones, results)
        except AssertionError:
...
class SyllableComplex1UnitCase(unittest.TestCase):
    ...
    def _test_single_vowel_syllable(self):
        ...
        try:
            run_test_cases(self.__sc1u, allophones, results)
        except AssertionError:
